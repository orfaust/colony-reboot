...
The ship catalog (`assets/config/default/ships.json`) now exposes:

- `sprite`: optional repository-relative PNG path string. Omitted or `""` means unset.
- `width`, `height`: required finite, strictly positive f32 dimensions in pixels at
  100% zoom (64 pixels per world unit), consistent with building dimensions.
  Fractional dimensions are valid. Values that overflow f32 or round to zero are rejected.
  Migration note: older catalogs stored these in world units, so multiply existing
  custom values by 64 (`1` becomes `64`).
...
The subject **type** catalog (`assets/config/default/subjects.json`) has the same optional
`sprite` path property and required `width`/`height` with the same finite positive
f32 pixel contract. These are not fields on individual level subjects. The
role catalog (`subject_roles.json`) has no color or sprite at all: subject
presentation resolves `subject.roles[].sprite` first, then this `subject.sprite`.
...
existing drawing dimensions and color. Subject drawing resolves
`subject.roles[].sprite` first and falls back to the subject type's own `sprite`,
then to its color; the role catalog itself contributes none of these. See
[Subject Role Assignments](subject-role-assignments.md).
...
The ship properties form exposes sprite, width and height, with pixel labels
and validation. New ships receive an empty sprite and dimensions 64×64. The subject
properties form includes sprite and pixel dimension fields; new types start
with an empty sprite and dimensions 64×64. Dimensions are also exposed by the catalog
grid. Forms
